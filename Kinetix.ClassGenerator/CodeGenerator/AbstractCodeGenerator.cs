    ...
    public abstract class AbstractCodeGenerator
    {
        ...
        private void GenerateDbContext(IEnumerable<ModelRoot> modelRootList)
        {
            ...
            using (_currentWriter = new CsharpFileWriter(targetFileName, csprojFileName))
            {
                ...
                WriteLine("using Kinetix.Database;");
        ...
        private void GenerateProperty(ModelProperty property)
        {
            ...
            if (GeneratorParameters.IsPostSharpDisabled)
            {
                ...
                if (property.DataDescription.Domain != null)
                {
                    ...
                    foreach (ValidationAttribute va in ((IDomainChecker)Singletons.DomainManager.GetDomain(domain.Code)).ValidationAttributes)
                    {
                        if (va != null)
                        {
        ...
        private void GenerateReferenceAccessorsImplementation(ICollection<ModelClass> classList, string projectName, string nameSpaceName)
        {
            ...
            using (_currentWriter = new CsharpFileWriter(implementationFileName, csprojFileName))
            {
                ...
                WriteLine(LoadUsing("Kinetix.Services.Annotations"));
                ...
                {
                    WriteLine(2, $"private readonly BrokerManager _brokerManager;");
                    WriteEmptyLine();
                    WriteSummary(2, "Constructeur");
                    WriteParam(2, "brokerManager", "BrokerManager");
                    WriteLine(2, $"public {implementationName}(BrokerManager brokerManager)");
                    WriteLine(2, "{");
                    WriteLine(3, "_brokerManager = brokerManager;");
                    WriteLine(2, "}");
                }
        ...
        private void GenerateUsing(ModelClass item)
        {
            ...
            {
                usingList.Add("Kinetix.ComponentModel.Annotations");
            }
