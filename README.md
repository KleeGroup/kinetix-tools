...
KTM attend un fichier de déclaration de domaines, par défaut appelé `"domains.yml"`, qui doit contenir l'ensemble des domaines utilisés par les propriétés de classes du modèle.
...
Il est conseillé de regrouper les classes selon une thématique métier commune. KTM considérera que tous les fichiers `*.yml` qu'il trouve, à l'exception du fichier de domaines, sont des fichiers de classes.
...
*Remarque : on ne gère pas de multiplicité sur les associations, elle ne se traduira directement que comme une foreign key, obligatoire ou non. Pas de n-n auto-générée non plus donc.*
...
Pour le lancer, il faudra utiliser la commande `dotnet run -p {chemin vers KTMG} -- {chemin du fichier de config} (--watch)`.

KTMG dispose de 5 générateurs, qui seront instanciés si la section de configuration associée est renseignée dans le fichier de config. Si vous avez déjà enregistré le schéma JSON du fichier de config depuis la présentation de KTM, vous devriez avoir les infos nécessaires pour configurer chacun des générateurs. Les 5 générateurs sont :
...
KTMG génère à très peu de choses près (de l'ordre du bugfix) la même chose que Kinetix.ClassGenerator. Leurs sorties respectives sont donc très comparables dans l'objectif de migrer de l'ancien vers le nouveau. La plupart du code a été directement réutilisée.
